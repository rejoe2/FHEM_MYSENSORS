...
sub Define($$) {
    ...
    #$hash->{retainedMessagesForRadioId} = {messages => []}; #initialise message array for smartSleep node
    AssignIoPort($hash);
};
...
sub Set($@) {
    ...
    $command eq "reboot" and do {
        ...
        sendClientMessage($hash, childId => 255, cmd => C_INTERNAL, subType => I_REBOOT) if (defined($hash->{OTA_BL_Type}) or $blVersion eq "3.0");
        last;
    };
...
sub Get($@) {
	...
	$command eq "RSSI" and do {
	    $hash->{I_RSSI} = 1; 
	    sendClientMessage($hash, cmd => C_INTERNAL, subType => I_SIGNAL_REPORT_REQUEST, payload => "R");
	    last;
	};
	$command eq "Extended_DEBUG" and do {
	    $hash->{I_DEBUG} = 1;
	    sendClientMessage($hash, cmd => C_INTERNAL, subType => I_DEBUG,payload => "F");
	    last;
	};
	$command eq "ReadingsFromComment" and do {
	    $hash->{getCommentReadings} = 1;
	    sendClientMessage($hash, cmd => C_INTERNAL, subType => I_PRESENTATION);
	    last;
	};
    }
}
...
sub onPresentationMessage($$) {
	...
	foreach my $type (@{$sensorMappings->{sends}}) {
		...
		if (defined (my $mapping = $hash->{readingMappings}->{$id}->{$type})) {
		    if ($mapping->{name} ne "mapReading_$typeStr$idStr" and $hash->{getCommentReadings} eq "2") {
			CommandDeleteReading(undef,"$hash->{NAME} $mapping->{name}");
			Log3 ($hash->{NAME}, 3, "MYSENSORS_DEVICE $hash->{NAME}: Deleted Reading $mapping->{name}");
		    }
		}
...
sub onInternalMessage($$) {
	...
	if ($hash->{I_DEBUG} == 1) {
	    ...
	    $hash->{I_DEBUG} = 2;
	    sendClientMessage($hash, childID => 255, cmd => C_INTERNAL, ack => 1, subType => I_DEBUG,payload => "V");
        } elsif ($hash->{I_DEBUG} == 2) {
	    readingsSingleUpdate($hash, "XDBG_CPU_VOLTAGE", $msg->{payload}, 1);
	    $hash->{I_DEBUG} = 3;
	    sendClientMessage($hash, childID => 255, cmd => C_INTERNAL, ack => 1, subType => I_DEBUG,payload => "M");
	...
	my $subSet = $hash->{I_RSSI};
	    ...
	    sendClientMessage($hash, cmd => C_INTERNAL, ack => 1, subType => I_SIGNAL_REPORT_REQUEST, payload => "R!");
        } elsif ($subSet == 2) {
	    readingsSingleUpdate($hash, "R_RSSI_from_Parent", $msg->{payload}, 1) if ($msg->{payload} ne "-256" );
	    $hash->{I_RSSI} = 3;
	    sendClientMessage($hash, cmd => C_INTERNAL, subType => I_SIGNAL_REPORT_REQUEST, payload => "S");
        } elsif ($subSet == 3) {
	    ...
	    $hash->{I_RSSI} = 5; 
	    sendClientMessage($hash, cmd => C_INTERNAL, subType => I_SIGNAL_REPORT_REQUEST, payload => "P");
        } elsif ($subSet == 5) {
	    readingsSingleUpdate($hash, "R_TX_Powerlevel_Pct", $msg->{payload}, 1) if ($msg->{payload} ne "-256" );
	    $hash->{I_RSSI} = 6; 
	    sendClientMessage($hash, cmd => C_INTERNAL, subType => I_SIGNAL_REPORT_REQUEST, payload => "T");
        } elsif ($subSet == 6) {
	    readingsSingleUpdate($hash, "R_TX_Powerlevel_dBm", $msg->{payload}, 1) if ($msg->{payload} ne "-256" );
	    $hash->{I_RSSI} = 7; 
	    sendClientMessage($hash, cmd => C_INTERNAL, subType => I_SIGNAL_REPORT_REQUEST, payload => "U");
        } elsif ($subSet == 7) {
...
sub sendClientMessage($%) {
	...
	unless (ref $messages eq 'ARRAY') {
	    ...
	    Log3 ($name,5,"$name: No array yet for enqueued message, building it!");
	} else {
